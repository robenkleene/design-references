...
1. For a final export, go to `out`, select the `mantra_ipr`, and set `Rendering > Sampling: 64` (from `9`). Also could try reducing the `Rendering > Noise Level` to lower than `0.01`.