...
3. With `Sphere Rim` selected, switch to the `Ellipse Tool` (`Q`), toggle on the grid with `⌥'`, then drag from the center to just around the energy ball, constraining proportions and sizing from the center by holding `⇧⌘`. Then toggle off the grid with `⌥'` again.

*The rest of this requires the free Video Copilot Saber effect*
